...
export class BroodjesService {
  ...
  getAllBroodjes(): Observable<Array<Broodje>> {
    return this.broodjesLijst$.asObservable();
  }

  withWantedIngredients(ingredienten: Array<string>): Observable<Array<Broodje>> {

    // stream of events --> stream of broodjes
    const subject = new Subject();

    subject.pipe(
      withLatestFrom(ingredienten, (data, ingredient) => ({data, ingredient})),
      filter(({data, ingredient}) =>  data.));



    this.broodjesLijst$.pipe(
      withLatestFrom(ingredienten, (data, ingredient ))
    );

    return this.broodjesLijst$.pipe(
      map(broodjes => broodjes.filter(broodje => broodje.ingredienten.find(e => e === ingredient) ))
    );


  }

  getBroodjes(ingredient: string): Observable<Array<Broodje>> {
    return this.broodjesLijst$.pipe(
      map(broodjes => broodjes.filter(broodje => broodje.ingredienten.find(e => e === ingredient) ))
      );
  }
